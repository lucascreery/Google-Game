...
import {Container, Row, Col, Navbar, NavbarBrand, Button, ListGroup, 
...
export default class GameScreen extends Component{
  ...
  nameSet(playerName) {
    ...
    this.socket.emit('new game', playerName);
  ...
  startGame(){
    this.socket.emit('start game', this.state.roomNum)
  }

  handleSubmitQuery(query){
    this.socket.emit('query', query)
    this.setState({
      screen: 'waiting query'
    })
  }
  ...
  render(){
    if(this.state.name){
        ...
        return(
          <div>
            <Navbar className="bg-dark">
              ...
              <Button className="mr-1 btn-raised" color="primary" onClick={this.startGame} style={{borderRadius: 10}}>Start Game</Button>
            </Navbar>
            <Container>
              <Row>
                <Col >
      ...
      }else if(this.state.screen === 'waiting query'){
        return(
          <div>
            <Navbar className="bg-dark">
              <NavbarBrand className="mr-auto text-light">Room: {this.state.roomNum}</NavbarBrand>
